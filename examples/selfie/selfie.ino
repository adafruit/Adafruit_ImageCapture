...
so think of it as a selfie camera. Tap "A" button to save image to card.
...
- OV7670 camera w/10K pullups to SDA+SCL, 3.3V+GND wires to shield
...
void loop() {
  ...
  if(CAM_MODE == ICAP_COLOR_YUV) {
    ...
    iCam_Y2RGB565(cam.getBuffer(), cam.width() * cam.height());
  }
