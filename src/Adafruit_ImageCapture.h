...
// work on arbitrary buffers, but are part of the library. The functions
// perform postprocessing on (currently) a 16-bit image, either RGB565 or
// sometimes YUV. These are not in-camera effects, though some might be
// possible to implement as such. Image is overwritten -- destination buffer
// is always the same as the source buffer, same dimensions, usually same
// colorspace. Not all functions work in all colorspaces.
