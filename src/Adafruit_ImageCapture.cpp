...
// These functions are preceded by "iCap" even though they're not tied to
// the camera hardware, just that they're part of the Image Capture lib.
...
void iCap_image_negative(uint16_t *pixels, uint16_t width, uint16_t height) {
...
void iCap_image_threshold(iCap_colorspace space, uint16_t *pixels,
                          uint16_t width, uint16_t height, uint8_t threshold) {
...
void iCap_image_posterize(iCap_colorspace space, uint16_t *pixels,
...
// Shower door effect.
void iCap_image_mosaic(iCap_colorspace space, uint16_t *pixels, uint16_t width,
                       uint16_t height, uint8_t tile_width,
                       uint8_t tile_height) {
...
void iCap_image_median(iCap_colorspace space, uint16_t *pixels, uint16_t width,
                       uint16_t height) {
  if (space == ICAP_COLOR_RGB565) {
    uint8_t *buf;
    ...
    if ((buf = (uint8_t *)malloc(buf_bytes_per_channel * 3))) {
      ...
      iCap_filter_row_prep(pixels, &rptr[1], width, buf_bytes_per_channel);
      ...
      iCap_filter_row_copy(&rptr[1], rptr, width + 2, buf_bytes_per_channel);
      ...
      for (y = 0; y < height; y++) { // For each row of image...
        ...
        if (y < (height - 1)) { // If current row is 0 to height-2
          ...
          iCap_filter_row_prep(&pixels[(y + 1) * width], &rptr[2], width,
          ...
          iCap_filter_row_copy(&rptr[1], &rptr[2], width + 2,
                               buf_bytes_per_channel);
        }
...
void iCap_image_edges(iCap_colorspace space, uint16_t *pixels, uint16_t width,
                      uint16_t height, uint8_t sensitivity) {
